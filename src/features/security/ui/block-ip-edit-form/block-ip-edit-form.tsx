...
import { useUpdateBlockIp } from '../../model/use-update-block-ip';
import { updateBlockIpSchema } from '../../model/update-block-ip.schema';
import { getApiErrorMessage } from '@shared/utils/api-error-message';
import type { z } from 'zod';
import styles from './block-ip-edit-form.module.css';

type UpdateFormValues = z.infer<typeof updateBlockIpSchema>;

interface BlockIpEditFormProps {
  id: number;
...
export default function BlockIpEditForm({ id, defaultValues, onSuccess, onCancel }: BlockIpEditFormProps) {
  const { control, handleSubmit, formState: { errors } } = useForm<UpdateFormValues>({
    resolver: zodResolver(updateBlockIpSchema),
    defaultValues,
  });
  const mutation = useUpdateBlockIp();
